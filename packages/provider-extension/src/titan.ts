...
import { TitanLogoBase64 } from "./constants";
import {
  Call,
...
export interface Result {
   ...
   * it should be considered and processed as `TitanError`.
   */
...
export class Titan implements ITitan {
  ...
  public readonly mode: TitanMode = "extension";

  public defaultOptions: TitanIntereactionOptions = {};

  static async getTitan(
    pingTimeout: number = 1500
  ): Promise<Titan | undefined> {
  ...
  async ping(): Promise<void> {
    await Titan.requestMethod("ping", []);
  }

  async enable(chainIds: string | string[]): Promise<void> {
    await Titan.requestMethod("enable", [chainIds]);
  }

  async disable(chainIds?: string | string[]): Promise<void> {
    await Titan.requestMethod("disable", [chainIds]);
  }
  ...
  async experimentalSuggestChain(chainInfo: ChainInfo): Promise<void> {
    ...
    await Titan.requestMethod("experimentalSuggestChain", [chainInfo]);
  }

  async getKey(chainId: string): Promise<Key> {
    return await Titan.requestMethod("getKey", [chainId]);
  }

  async getKeysSettled(chainIds: string[]): Promise<SettledResponses<Key>> {
    return await Titan.requestMethod("getKeysSettled", [chainIds]);
  }
  ...
  async sendTx(
    ...
    return await Titan.requestMethod("sendTx", [chainId, tx, mode]);
  }
  ...
  async signAmino(
    ...
    return await Titan.requestMethod("signAmino", [
      chainId,
      signer,
      signDoc,
  ...
  async signDirect(
    ...
    const result = await Titan.requestMethod("signDirect", [
      chainId,
      signer,
  ...
  async signDirectAux(
    ...
    const result = await Titan.requestMethod("signDirectAux", [
      chainId,
      signer,
      // We can't send the `Long` with remaing the type.
      // Receiver should change the `string` to `Long`.
  ...
  async signArbitrary(
    ...
    return await Titan.requestMethod("signArbitrary", [chainId, signer, data]);
  }
  ...
  signICNSAdr36(
    ...
    return Titan.requestMethod("signICNSAdr36", [
      chainId,
      contractAddress,
      owner,
      username,
  ...
  async verifyArbitrary(
    ...
    return await Titan.requestMethod("verifyArbitrary", [
      chainId,
      signer,
      data,
      signature,
  ...
  async signEthereum(
    ...
    return await Titan.requestMethod("signEthereum", [
      chainId,
      signer,
      data,
      type,
  ...
  async suggestToken(
    ...
    return await Titan.requestMethod("suggestToken", [
      chainId,
      contractAddress,
      viewingKey,
    ]);
  ...
  async getSecret20ViewingKey(
    ...
    return await Titan.requestMethod("getSecret20ViewingKey", [
      chainId,
      contractAddress,
    ]);
  }
  ...
  async getEnigmaPubKey(chainId: string): Promise<Uint8Array> {
    return await Titan.requestMethod("getEnigmaPubKey", [chainId]);
  }
  ...
  async getEnigmaTxEncryptionKey(
    ...
    return await Titan.requestMethod("getEnigmaTxEncryptionKey", [
      chainId,
      nonce,
    ]);
  }
  ...
  async enigmaEncrypt(
    ...
    return await Titan.requestMethod("enigmaEncrypt", [
      chainId,
      contractCodeHash,
      msg,
    ]);
  ...
  async enigmaDecrypt(
    ...
    return await Titan.requestMethod("enigmaDecrypt", [
      chainId,
      ciphertext,
      nonce,
    ]);
  ...
  async experimentalSignEIP712CosmosTx_v0(
    ...
    return await Titan.requestMethod("experimentalSignEIP712CosmosTx_v0", [
      chainId,
      signer,
      eip712,
      signDoc,
  ...
  async getChainInfosWithoutEndpoints(): Promise<ChainInfoWithoutEndpoints[]> {
    return await Titan.requestMethod("getChainInfosWithoutEndpoints", []);
  }
  ...
  async changeKeyRingName({
    ...
    return await Titan.requestMethod("changeKeyRingName", [
      { defaultName, editable },
    ]);
  }

  async sendEthereumTx(chainId: string, tx: Uint8Array): Promise<string> {
    return await Titan.requestMethod("sendEthereumTx", [chainId, tx]);
  }

  async suggestERC20(chainId: string, contractAddress: string): Promise<void> {
    return await Titan.requestMethod("suggestERC20", [
      chainId,
      contractAddress,
    ]);
  }
  ...
  async getChainInfoWithoutEndpoints(
    ...
    return await Titan.requestMethod("getChainInfoWithoutEndpoints", [chainId]);
  }
  ...
  async getStarknetKey(chainId: string): Promise<{
    ...
    return await Titan.requestMethod("getStarknetKey", [chainId]);
  }
  ...
  > {
    return await Titan.requestMethod("getStarknetKeysSettled", [chainIds]);
  }
  ...
  async signStarknetTx(
    ...
    return await Titan.requestMethod("signStarknetTx", [
      chainId,
      transactions,
      details,
    ]);
  ...
  async signStarknetDeployAccountTransaction(
    ...
    return await Titan.requestMethod("signStarknetDeployAccountTransaction", [
      chainId,
      transaction,
    ]);
  }
...
class EthereumProvider extends EventEmitter implements IEthereumProvider {
  protected readonly eip6963ProviderInfo?: EIP6963ProviderInfo = {
    ...
    name: "Titan",
    rdns: "app.titan",
    icon: `data:image/png;base64,${TitanLogoBase64}`,
  };
  ...
  constructor(protected readonly titan: Titan) {
    super();

    window.addEventListener("titan_keystorechange", async () => {
      if (this._currentChainId) {
        const chainInfo = await titan.getChainInfoWithoutEndpoints(
          this._currentChainId
        );

        if (chainInfo) {
          const selectedAddress = (await titan.getKey(this._currentChainId))
            .ethereumHexAddress;
