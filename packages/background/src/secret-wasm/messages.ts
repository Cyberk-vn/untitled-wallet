...
export class GetPubkeyMsg extends Message<Uint8Array> {
  ...
  validateBasic(): void {
    if (!this.chainId) {
      throw new TitanError("secret-wasm", 100, "chain id not set");
    }
  }
...
export class ReqeustEncryptMsg extends Message<Uint8Array> {
  ...
  validateBasic(): void {
    ...
    if (!this.contractCodeHash) {
      throw new TitanError("secret-wasm", 103, "contract code hash not set");
    }

    if (!this.msg) {
      throw new TitanError("secret-wasm", 101, "msg not set");
    }
  }
...
export class RequestDecryptMsg extends Message<Uint8Array> {
  ...
  validateBasic(): void {
    ...
    if (!this.cipherText || this.cipherText.length === 0) {
      throw new TitanError("secret-wasm", 102, "ciphertext not set");
    }

    if (!this.nonce || this.nonce.length === 0) {
      throw new TitanError("secret-wasm", 110, "nonce not set");
    }
  }
...
export class GetTxEncryptionKeyMsg extends Message<Uint8Array> {
  ...
  validateBasic(): void {
    ...
    if (!this.nonce) {
      ...
      throw new TitanError("secret-wasm", 111, "nonce is null");
    }
  }
